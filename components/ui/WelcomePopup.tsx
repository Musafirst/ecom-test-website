...
export function WelcomePopup() {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem(STORAGE_KEY)) return    // already claimed — never show again
    if (sessionStorage.getItem(SESSION_KEY)) return  // dismissed this session — skip
  ...
  function handleClose() {
    sessionStorage.setItem(SESSION_KEY, '1')  // don't show again this session
    setVisible(false)
  }
    ...
    <AnimatePresence>
      {visible && (
        <>
          {/* Backdrop */}
          <motion.div
            className="fixed inset-0 z-[200] bg-jamm-dark/40 backdrop-blur-sm"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-1/2 z-[201] w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-jamm-gold/20 bg-[#FAF7F2] p-8 shadow-[0_24px_80px_rgba(12,11,9,0.22)]"
            initial={{ opacity: 0, scale: 0.93, y: 12 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.93, y: 12, transition: { duration: 0.22, ease: [0.32, 0.72, 0, 1] } }}
            ...
            {!claimed ? (
              ...
              <div className="mt-6">
                ...
                <p className="mt-3 font-sans text-xs leading-relaxed text-jamm-dark/45">
                  Apply this code at checkout to save 20% on your first order. Valid for new customers only.
                </p>
